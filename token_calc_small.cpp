    ...
    while (scanf("%d%d",&row,&line)!=EOF)
    {
		...
		else
        {
            ...
            while(sum>0)
            {
                ...
                for(int i=0;i<num;i++)
                {
                    startpoint[i].x=import[i+cnt*unit].a;
                    startpoint[i].y=import[i+cnt*unit].b;
                    endpoint[i].x=import[i+cnt*unit].c;
                    endpoint[i].y=import[i+cnt*unit].d;
                    now[i][0]=startpoint[i];
                    graph_inital();
                    find_shortest(i);
                    step_contain[i+cnt*unit]=step_total[i];
                }
                init();
                time=0;
                tmp_t[cnt]=0;
