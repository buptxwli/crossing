...
void calc_at_crossing_zone(int t,int z)  //new
{
    at_zone_size[z]=0;
    for(int i=0;i<num;i++)
    {
...
void end_release(int t)   //off_crossing_zone  new
{
    for(int i=0;i<num;i++)
    {
...
void go_release(int t) //new
{
    for(int i=0;i<num;i++)
    {
        if(now[i][t]==now[i][t-1])
            continue;
        else if(in[now[i][t-1].x][now[i][t-1].y]=='X' ||in[now[i][t-1].x][now[i][t-1].y]=='Y')
            continue;
